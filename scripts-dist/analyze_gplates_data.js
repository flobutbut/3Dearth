...
class GplatesAnalyzer {
    constructor() {
        this.baseUrl = 'https://gws.gplates.org/webservice/reconstruct_features/';
        this.outputDir = path.join(__dirname, '../data/gplates');
    ...
    async analyzePlateBoundaries(time) {
        try {
            ...
            await fs.writeFile(path.join(this.outputDir, `boundaries_${time}Ma.json`), JSON.stringify(analysis, null, 2));
    ...
    async analyzePlateVelocities(time) {
        try {
            ...
            await fs.writeFile(path.join(this.outputDir, `velocities_${time}Ma.json`), JSON.stringify(analysis, null, 2));
            console.log('Analyse terminée !');
    ...
    async analyzeRotations(time) {
        try {
            ...
            await fs.writeFile(path.join(this.outputDir, `rotations_${time}Ma.json`), JSON.stringify(analysis, null, 2));
            console.log('Analyse terminée !');
            console.log(`Nombre de plaques analysées : ${features.length}`);
        }
        catch (error) {
