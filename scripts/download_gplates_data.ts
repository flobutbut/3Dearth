...
interface GpmlzData {
  ...
  polygons: GpmlzGeometry[];
}

class GplatesDownloader {
  private outputDir: string;
  private datasets: GplatesDataset[];
  ...
  async parseRotationFile(filePath: string): Promise<any> {
    try {
      const content = await fs.readFile(filePath, 'utf-8');
      const lines = content.split('\n');
      const rotations = [];
      const plateRotations: Record<number, any[]> = {};
  ...
  async parseGpmlzFile(filePath: string): Promise<GpmlzData> {
    try {
      ...
      for (const featureMember of features) {
        ...
        const plateId = parseInt(shapefileAttributes.find((e: any) => e['gpml:KeyValueDictionaryElement']?.[0]?.['gpml:key']?.[0] === 'PLATEID1')?.['gpml:KeyValueDictionaryElement']?.[0]?.['gpml:value']?.[0] || '0');
        const conjugatePlateId = parseInt(shapefileAttributes.find((e: any) => e['gpml:KeyValueDictionaryElement']?.[0]?.['gpml:key']?.[0] === 'PLATEID2')?.['gpml:KeyValueDictionaryElement']?.[0]?.['gpml:value']?.[0] || '0');
        const name = shapefileAttributes.find((e: any) => e['gpml:KeyValueDictionaryElement']?.[0]?.['gpml:key']?.[0] === 'NAME')?.['gpml:KeyValueDictionaryElement']?.[0]?.['gpml:value']?.[0] || featureType;
        
        if (plateId) plateIds.add(plateId);
        if (conjugatePlateId) plateIds.add(conjugatePlateId);
