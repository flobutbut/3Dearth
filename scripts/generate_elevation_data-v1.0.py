...
def process_elevation_data():
    ...
    # Terres émergées
    elevation[land_mask] = (
        500 +  # Élévation de base des continents plus basse
        7000 * (continents[land_mask] - land_threshold) / (1 - land_threshold) +  # Élévation continentale
        1500 * terrain_detail[land_mask]  # Détails du terrain
    ).astype(np.float32)
    
    # Océans
    ocean_elevation = (
        -3500 +  # Profondeur moyenne des océans
        -6500 * (land_threshold - continents[~land_mask]) / land_threshold +  # Variation de profondeur
        1000 * ocean_detail[~land_mask]  # Détails du fond océanique
    ).astype(np.float32)
    elevation[~land_mask] = ocean_elevation
    
    # Assurer que les valeurs restent dans des limites réalistes
    elevation = np.clip(elevation, -11000, 8848)  # Entre fosse des Mariannes et Mont Everest
    
    # Ajouter un effet de latitude (plus de montagnes près de l'équateur)
    lat = np.linspace(-90, 90, height)
    lat_factor = np.ones((height, width))
    for i in range(height):
        # Effet plus prononcé sur les terres que sur les océans
        lat_effect_land = 1.0 - 0.3 * abs(lat[i]) / 90.0
        lat_effect_ocean = 1.0 - 0.1 * abs(lat[i]) / 90.0
